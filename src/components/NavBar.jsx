            ...
            >
              {!open && (
                <>
                  <span
                    aria-hidden="true"
                    className="font-mono text-base text-fuchsia-600"
                  >
          ...
          {sections.map((section, sectionIndex) => (
            <li key={section.id} className="flex [counter-increment:section]">
              <Link
                href={`#${section.id}`}
                className={clsx(
                  'flex w-full flex-col items-center justify-center border-b-2 before:mb-2 before:font-mono before:text-base before:content-[counter(section,decimal-leading-zero)]',
                  sectionIndex === activeIndex
                    ? 'border-fuchsia-600 bg-fuchsia-50 text-fuchsia-600 before:text-fuchsia-600'
                    : 'border-transparent before:text-stone-500 hover:bg-fuchsia-50/40 hover:before:text-stone-900'
                )}
              >
                {section.title}
              </Link>
